...
const verifyScoringPermission = (match, req) => {
  ...
  const isCreator = match.created_by?.toString() === userId;
  const isScorer = match.scorers?.some(id => id.toString() === userId);
  const playerProfileId = req.user.associatedPlayerId;
  const isUmpire = playerProfileId && match.umpires?.some(id => id.toString() === playerProfileId);
  return isCreator || isScorer || isUmpire;
};
